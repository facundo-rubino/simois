    ...
    >
      ...
      {servicio.destacado && (
        <div className="absolute top-4 right-4 z-10">
          <span className="inline-flex items-center gap-1 px-3 py-1 bg-simois-orange text-white text-xs font-medium rounded-full">
      ...
      <div className="aspect-[4/3] overflow-hidden relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
          <span className="text-simois-orange text-xs font-medium uppercase tracking-wider">
            {servicio.categoria}
          </span>
        </div>
      </div>
        ...
        {servicio.detalles && (
          <ul className="space-y-2 mb-6">
            {servicio.detalles.map((detalle, index) => (
              <li key={index} className="flex items-center gap-2 text-sm text-simois-dark/70">
